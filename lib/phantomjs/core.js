	...
	page.onCallback = function(callbackObject) {
	  debug('phantom.onCallback');
		...
		if (callbackObject.status === NORMALIZATION_DONE) {
			...
			prepareNewPage()
			critialCssOptions.ast = cssAstFormatter.parse(callbackObject.css, { silent: true });
			getCriticalPathCss(critialCssOptions)
			return
		}
	...
	try {
		if (criticalRules && criticalRules.length > 0) {
			...
			if(finalCss.trim().length === 0){
				errorlog('Note: Generated critical css was empty for URL: ' + critialCssOptions.url);
			}
...
function extractFullCssFromPage(doneStatus, css) {
	...
	css = Array.prototype.map.call(document.styleSheets, function (stylesheet) {
...
var css
try {
	var f = fs.open(critialCssOptions.css, 'r')
	css = f.read()
} catch (e) {
	errorlog(e);
	phantomExit(1);
...
prepareNewPage()
if (critialCssOptions.strict) {
	try {
		critialCssOptions.ast = cssAstFormatter.parse(css);
	} catch (e) {
		errorlog(e.message)
		phantomExit(1)
	}
