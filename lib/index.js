...
function penthouseScriptArgs (options) {
...
function generateCriticalCss (scriptArgs, options, callback) {
  var stdOut = '',
    stdErr = '',
    debuggingHelp = '',
    cp,
  ...
  var scriptArgs = penthouseScriptArgs(options)
...
var m = module.exports = function (options, callback) {
  ...
  var ast
  try {
    css = fs.readFileSync(options.css)
    // debuglog('opened css from file')
  } catch (e) {
    // TODO
    console.error('errors opening css file: ' + options.css + e.toString())
    process.exit(1)
  }
  try {
    ...
    normalizeCss(
      ...
      function (err, normalizedCss) {
        ...
        // TODO
        // console.log('now parse ast')
        // ast = cssAstFormatter.parse(normalizedCss)

        // then save ast to file
        var tmpobj = tmp.fileSync({dir: '/tmp'})
        fs.writeFileSync(tmpobj.name, normalizedCss)

        // then call phantom for critical css
        var scriptArgs = penthouseScriptArgs(options)
        scriptArgs[1] = tmpobj.name
        generateCriticalCss(scriptArgs, options, callback)
      }
    )
  }

  if (ast) {
    // TODO: create tmp file with ast
    console.log('save ast to file, call phantom')
    // scriptArgs[1] = astFilePath
    // generateCriticalCss(scriptArgs)
  }
}
