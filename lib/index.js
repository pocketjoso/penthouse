...
let generateAstFromCssFile = (() => {
  var _ref = _asyncToGenerator(function* (options, { debuglog, stdErr }) {
    // read the css and parse the ast
    // if errors, normalize css and try again
    // only then pass css to penthouse
    let css;
    try {
      css = yield readFilePromise(options.css, 'utf8');
    } catch (e) {
      throw e;
    }
  ...
  return function generateAstFromCssFile(_x, _x2) {
    return _ref.apply(this, arguments);
  };
})();

let generateCriticalCss = (() => {
  var _ref2 = _asyncToGenerator(function* (options, ast, { debuglog, stdErr, START_TIME }) {
    ...
    } = yield writeToTmpFile(JSON.stringify(ast));

    const scriptArgs = penthouseScriptArgs(options, astFilePath);
    const phantomJsArgs = [configString, ...toPhantomJsOptions(options.phantomJsOptions), script, ...scriptArgs];
    ...
    return new Promise(function (resolve, reject) {
      ...
      cp.on('exit', function (code) {
        if (code === 0) {
          ...
          const formattedCss = postformatting(stdOut, {
  ...
  return function generateCriticalCss(_x3, _x4, _x5) {
...
const m = module.exports = function (options, callback) {
  ...
  return generateAstFromCssFile(options, logging).then(ast => generateCriticalCss(options, ast, logging)).then(criticalCss => {
    if (callback) {
